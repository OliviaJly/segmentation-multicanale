...
base_test2 =  quanti_trans.drop(['IDPART_CALCULE','nb_jours_CAEL_3m','nb_jours_MaBanque_3m','Actionsd_MaBanque_3m',
...
data_clus['count'] = data_clus.groupby('cluster')['cluster'].transform('count') # Compte le nombre d'
# observations dans 1 cluster
data_clus_10 = data_clus[data_clus['count']>10] # Garder uniquement ceux qui contiennet plus de 10 observations
centroid=data_clus_10.groupby('cluster').mean() # Centroids par cluster
centroid2=np.array(centroid.drop('count',axis=1)) # Centroids contains the centroids of the clusters that gather
...
## Relancer un k-means en virant les observations dont la distance au cluster le plus proche est supérieure à 5
## et en initialisant les centres précédents
...
# Code pour boxplots
sub= list(range(1, 38, 1))
plt.figure(figsize=(40,40))
var_names = list(clustered_data.columns.values)[0:37]
for i in range(0,37):
...
C = np.array(pd.concat([pd.DataFrame(np.array(data_coor2)[:,0:10]),clustered_data['cluster']],axis=1))

fig = plt.figure(1, figsize=(8, 6))
ax = Axes3D(fig, elev=-150, azim=110)
ax.scatter(C[:, 0], C[:, 1], C[:, 2], c=(C[:,10] *2),cmap=plt.cm.Paired)
