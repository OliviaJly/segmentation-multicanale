...
clusters = pd.read_csv(path + '/OLIVIA_CLUST12_nont.csv', delimiter=";") 

# clustering obtenu sur SAS avec var quanti tranformées
clusterst = pd.read_csv(path + '/OLIVIA_CLUST12.csv', delimiter=";") 
...
pd.crosstab(prev_clusters_0916['cluster_coded'].values+1,clusters_0916['cluster'].values+1)
pd.crosstab(prev_clusters_0916['cluster_coded'].values+1,clusters_0916['cluster'].values+1).apply(lambda r: r/r.sum(), axis=1)
...
ax.scatter(C[C[:, 10] == 1, 2], C[C[:, 10] == 1, 0], C[C[:, 10] == 1, 1], \
           c='royalblue', cmap=plt.cm.Paired, label='Classe 1 (CAEL)')
ax.scatter(C[C[:, 10] == 2, 2], C[C[:, 10] == 2, 0], C[C[:, 10] == 2, 1], \
           c='forestgreen', cmap=plt.cm.Paired, label='Classe 2 (Ma Banque)')
ax.scatter(C[C[:, 10] == 3, 2], C[C[:, 10] == 3, 0], C[C[:, 10] == 3, 1], \
           c='firebrick', cmap=plt.cm.Paired, label='Classe 3 (Retraités)')
ax.scatter(C[C[:, 10] == 4, 2], C[C[:, 10] == 4, 0], C[C[:, 10] == 4, 1], \
           c='slateblue', cmap=plt.cm.Paired, label='Classe 4 (Inactifs)')
...
def codage(x):
    if x['prev_cluster']==1 and x['cluster']==2 :
       return 4
    else :
        return x['cluster']
...
#fusion des 4 freq 
evol_freq = pd.concat([freq_1215, freq_0316, freq_0616, freq_0916], axis=1)
